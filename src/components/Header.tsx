    ...
    <header className="bg-white shadow-md p-4 flex justify-between items-center">
      ...
      <div className="relative">
        ...
        >
          ...
          {itemCount > 0 && (
            <span className="absolute -top-2 -right-2 bg-red-500 text-white text-xs w-5 h-5  flex items-center justify-center rounded-r-full">
              {itemCount}
            </span>
          )}
        </button>
          ...
          <div className="absolute right-0 mt-2 w-80 bg-white border rounded shadow-lg z-50">
            <div className="p-4">
              ...
              {cart.length === 0 ? (
                ...
                <>
                  <ul className="max-h-60 overlow-y-auto divide-y divide-gray-200">
                    {cart.map(item => (
                      <li
                        key={item.id}
